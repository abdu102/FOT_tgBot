...
export function sessionViewScene(prisma: PrismaClient) {
  ...
  (scene as any).action?.(/sess_start_(.*)/, async (ctx: any) => {
    ...
    try {
      ...
      // Try to update the current view inline so admin sees the change instantly
  ...
  (scene as any).action?.(/sess_m_(.*)/, async (ctx: any) => {
    ...
    const kb = {
      inline_keyboard: [
        [{ text: '🏆 1 g\'alaba', callback_data: `m_res_${mid}_1` }, { text: '🏆 2 g\'alaba', callback_data: `m_res_${mid}_2` }, { text: '🤝 Durang', callback_data: `m_res_${mid}_D` }],
        [{ text: '⬅️ Orqaga', callback_data: `sess_matches_${sid || ''}` }],
      ],
    } as any;
  ...
  (scene as any).action?.(/m_res_(.*)_(1|2|D)/, async (ctx: any) => {
    ...
    await prisma.match.update({ where: { id: mid }, data } as any);
    try { await ctx.answerCbQuery('Saved'); } catch {}
  });
  (scene as any).action?.(/m_goal_(.*)/, async (ctx: any) => {
    ...
    rows.push([{ text: '⬅️ Back', callback_data: `sess_m_${mid}` }]);
